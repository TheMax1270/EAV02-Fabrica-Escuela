...
BEGIN
    ...
    END IF;
    IF (SELECT count(*) FROM devconnect_test_user_ids) >= (SELECT count(*) FROM users.app_users) THEN
        RAISE EXCEPTION 'Refusing to delete every user';
    END IF;
END
$$;

DELETE FROM authentication.auth_sessions
WHERE user_id IN (SELECT id FROM devconnect_test_user_ids);

DELETE FROM users.app_users
